...
**Plans**: 13 plans (6 original + 09-07..09-11 gap closure + 09-12/09-13 UAT gap closure)
Plans:
...
- [ ] 10-11-PLAN.md — Deploy + live E2E: systemd unit + reverse-proxy config + full-chain human verification (login→edit→publish→render, non-editor denied, role-loss unpublish) + key rotation — **Task 1 DONE 2026-07-15** (deploy artifacts committed: nocturna-bot 0ae4ff0 — `deploy/nocturna-editor-admin.service` uvicorn/127.0.0.1:8770 + `deploy/Caddyfile.snippet` auto-HTTPS + `EDITOR_DEPLOY.md §7` runbook, no secret literals). **Task 2 (live E2E human-verify) + Task 3 (record verification + rotation) PENDING** — require real cinema SSH + browser + Discord OAuth; see `10-11-SUMMARY.md` "LIVE VERIFICATION CHECKLIST". Phase is code-complete + deploy-ready but NOT live-verified.
